...
func play_animation(animation_name: String, loop: bool, track: int, reverse: bool = false, additive: bool = false, delay: float = 0.00):
	...
	if current_animation != animation_name:
		...
		if additive == false:
			track_entry = SPINE.get_animation_state().set_animation(animation_name, loop, track)
		else:
			track_entry = SPINE.get_animation_state().add_animation(animation_name, delay, loop, track)
